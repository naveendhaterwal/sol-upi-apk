...
export function SolanaLogo(props: LogoProps) {
  return <PublicLogo src="/solana.png" {...props} />;
}
export function UsdcLogo(props: LogoProps) {
  return <PublicLogo src="/usdc.png" {...props} />;
}
...
export function NoriWordmark({
  ...
  return (
    <span
      ...
      aria-label={title ?? "SolUPI"}
    >
      <SolanaLogo className={cn("size-7", markClassName)} />
      <span
        className={cn(
          "text-[26px] font-semibold tracking-tight leading-none text-foreground",
          textClassName,
        )}
      >
        SolUPI
